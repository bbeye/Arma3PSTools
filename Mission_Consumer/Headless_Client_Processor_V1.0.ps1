    ...
    CHECK Create new directory as "PBOname" and extract files to
        Following scripts use $FileLocation set by detected script
            CHECK Zip file is archived, other files placed in needed locations:
                  Modlist > D:\Arma3\configs\iceberg_sideops
                  Mission file > D:\Arma3\missions\iceberg_sideops\
...
#create the task name
    ...
    #something like: 
        $TaskName = (Get-ChildItem $ZipFileDirName *.pbo -Recurse).Name.Split(".",2)[0] + " on " + (Get-ChildItem $ZipfileDirname modlist*.* -Recurse).Name.Split("_",2)[-1]
        $ReceiptName = (Get-ChildItem $ZipFileDirName *.pbo -Recurse).Name.Split(".",2)[0]
...
#ACTIONS
    ...
    $TaskAction = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "Start-Arma3Server -launchID iceberg_sideops -Port 2802 -modlistSelect DateAware -runType Server"
...
#TRIGGERS
    #Pull this via name of modfile
        ...
        $ModlistFileName = Get-ChildItem $ZipfileDirname modlist*.* -Recurse | Split-Path -leaf
...
#create the task
    try{
    Register-ScheduledTask -Action $TaskAction -TaskName "$TaskName" -TaskPath "\Arma3\sideops Schedules\" -Trigger $TaskTrigger1 -Settings $TaskSettings -Force
    } catch {
    Clean-AutomationFolder
    Stop-Transcript
    Exit
