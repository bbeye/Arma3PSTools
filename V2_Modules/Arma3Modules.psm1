...
function Update-Arma3Mod {
    ...
    foreach ($line in (gc .\modlistextract.txt)) {
        ...
        {     
            ...
            $modnumber = $line | Select-String -Pattern '(?<=\bid=)\d+' | ForEach-Object { $_.Matches.Value } 
            $runBit += 1
            ...
            #$modName = $line | Select-String -Pattern '<div class="requiredItem">\s*([^<]+)\s*</div>' | ForEach-Object { $_.Matches.Groups[1].Value }
            $runBit += 1
