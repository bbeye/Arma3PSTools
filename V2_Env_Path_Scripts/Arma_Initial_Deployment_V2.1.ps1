...
Get-Arma3ShadowMod -shadowModName "$shadowModName" -shadowModNumber "$shadowModNumber" |Out-File "$($armaDir)configs\$serverName\modlist.txt"
...
switch ($configBit) 
    ...
    1   {
        ...
        $basicConfig = Get-FileName("$($env:ArmaPath)") | 
        ...
        $serverConfig = Get-FileName("$($env:ArmaPath)") 
    ...
    2   {
        ...
        Copy-Item -path "$($env:ArmaPath)Scripts\Arma3PSTools\Default_Configs\Default_basic.cfg" -destination "$($env:ArmaPath)Configs\$serverName\$($serverName)_Basic.cfg"
        Copy-Item -path "$($env:ArmaPath)Scripts\Arma3PSTools\Default_Configs\Default_server.cfg" -destination "$($env:ArmaPath)Configs\$serverName\$($serverName)_server.cfg"
